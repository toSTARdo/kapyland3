...
class AnimalRepository:
    ...
    async def upsert(self, animal: Animal):
        async with self.pool.acquire() as conn:
            ...
            state_json = json.dumps(animal.state, ensure_ascii=False)
            
            equip_data = {}
            for slot, item in animal.equipment.items():
                if item:
                    equip_data[slot] = item.model_dump() if hasattr(item, 'model_dump') else item
                else:
            ...
            await conn.execute("""
                INSERT INTO capybaras (
                    owner_id, name, race, lvl, exp, weight, hp, atk, def, agi, luck, stamina, 
                    max_stamina, inventory, equipment, state, ship_id
            ...
            """, 
            animal.owner_id, animal.name, animal.race, animal.level, animal.exp, animal.weight,
            animal.stats.hp, animal.stats.atk, animal.stats.def_, 
            animal.stats.agi, animal.stats.luck, animal.stats.stamina,
            animal.stats.max_stamina, inv_json, equip_json, state_json, animal.ship_id)